...
namespace WebApi.Shared.DataTransferObjects
{
    ...
    public record ProductForCreationDto(Category Category, string Name, string Description, ICollection<ImageUrl> ImageURLs, int Price, int Discount_Price, int Rating, ICollection<Review> Reviews, int Total_Sell, int Stock);
}
