...
public class AdminsController : ControllerBase
{
    ...
    public AdminsController(IServiceManager service, HttpClient client, IConnectionMultiplexer muxer, IDistributedCache cache)
    {
        _service = service;
    ...
    public async Task<IActionResult> UpdateAdmin(Guid id, AdminForUpdateDto adminUpdate)
    {
        try
        {
            ...
            if (adminUpdate is null)
                return BadRequest("AdminForUpdateDto object is null");

            if (!ModelState.IsValid)
                return UnprocessableEntity(ModelState);

            await _service.UserService.UpdateAdminAsync(id, adminUpdate, trackChanges: true);
