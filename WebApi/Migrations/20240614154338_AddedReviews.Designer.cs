// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using WebApi.Repository;

#nullable disable

namespace WebApi.Migrations
{
    [DbContext(typeof(RepositoryContext))]
    [Migration("20240614154338_AddedReviews")]
    partial class AddedReviews
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.2")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = "43023697-2e80-4897-bd7e-1af6210a6dad",
                            Name = "Manager",
                            NormalizedName = "MANAGER"
                        },
                        new
                        {
                            Id = "ab971ffc-f154-4d5f-b321-b1f4bac6d308",
                            Name = "Administrator",
                            NormalizedName = "ADMINISTRATOR"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("Model.Category", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("ImageUrl");

                    b.Property<string>("SubTitle")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("SubTitle");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Title");

                    b.HasKey("Id");

                    b.ToTable("Categories", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("b5ebb95a-9fdd-40c9-b077-1da0d5167884"),
                            ImageUrl = "https://cdn.shopify.com/s/files/1/1706/9177/products/NEWAppleMacbookProwithM1ProChip14InchLaptop2021ModelMKGQ3LL_A_16GB_1TBSSD_custommacbd.jpg?v=1659592838",
                            SubTitle = "",
                            Title = "Computers and Laptops"
                        },
                        new
                        {
                            Id = new Guid("9f725793-ac1b-4cee-bd8d-ecc4cd892829"),
                            ImageUrl = "https://indian-retailer.s3.ap-south-1.amazonaws.com/s3fs-public/2021-07/kosme1.png",
                            SubTitle = "",
                            Title = "cosmetics and body care"
                        },
                        new
                        {
                            Id = new Guid("e4776445-863e-4555-b348-a3c865182f34"),
                            ImageUrl = "https://img.freepik.com/free-vector/ordering-goods-online-internet-store-online-shopping-niche-e-commerce-website-mother-buying-babies-clothes-footwear-toys-infant-accessories_335657-2345.jpg?w=2000",
                            SubTitle = "",
                            Title = "Accesories"
                        },
                        new
                        {
                            Id = new Guid("0c96ac86-d8dd-4e3a-80fa-b408a528d11c"),
                            ImageUrl = "https://www.shift4shop.com/2015/images/industries/clothing/clothing-apparel.png",
                            SubTitle = "",
                            Title = "Cloths"
                        },
                        new
                        {
                            Id = new Guid("926b1d03-ee7e-4a83-b452-c40f3c13946b"),
                            ImageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQvBQPQMVNRd6TtDkGs2dCri0Y-rxKkFOiEWw&usqp=CAU",
                            SubTitle = "",
                            Title = "Shoes"
                        },
                        new
                        {
                            Id = new Guid("e1fa6503-dbc6-4f9a-9663-9cec1690dd5e"),
                            ImageUrl = "https://searchspring.com/wp-content/uploads/2022/10/Hero-Image-Platform-Others-2.png",
                            SubTitle = "",
                            Title = "Gifts"
                        },
                        new
                        {
                            Id = new Guid("1a55c4b0-3bb7-4aef-b491-b89370a4809c"),
                            ImageUrl = "https://cdn.openpr.com/T/c/Tc15444071_g.jpg",
                            SubTitle = "",
                            Title = "Pet Care"
                        },
                        new
                        {
                            Id = new Guid("a82f0939-9d70-4b8b-8548-db1886df45cc"),
                            ImageUrl = "https://st-troy.mncdn.com/mnresize/1500/1500/Content/media/ProductImg/original/mpwp3tua-apple-iphone-14-256gb-mavi-mpwp3tua-637986832343472449.jpg",
                            SubTitle = "",
                            Title = "Mobile and Tablets"
                        },
                        new
                        {
                            Id = new Guid("2122c1ea-a9ba-44e5-aee4-818ba4d7b01c"),
                            ImageUrl = "https://static.vecteezy.com/system/resources/previews/011/996/555/original/3d-black-headphone-illustration-ecommerce-icon-png.png",
                            SubTitle = "",
                            Title = "Music and Gaming"
                        },
                        new
                        {
                            Id = new Guid("6fbd61fc-0f67-4143-80ac-f68e85c660fd"),
                            ImageUrl = "",
                            SubTitle = "",
                            Title = "Others"
                        });
                });

            modelBuilder.Entity("Model.ImageUrl", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("ProductId");

                    b.Property<string>("PublicId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("PublicId");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Url");

                    b.HasKey("Id");

                    b.ToTable("ImageUrls", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("a6f5ad8b-bce8-45a5-b8d3-e00b96912ed1"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.istorebangladesh.com/images/thumbs/0000286_macbook-pro-m1_550.png"
                        },
                        new
                        {
                            Id = new Guid("992ecfb6-93ac-4ff3-a24c-facce65500cc"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.custommacbd.com/cdn/shop/products/AppleMacBookProwithM2chip2022model_8GB_256GB_CustomMacBD_540x.jpg?v=1659438499"
                        },
                        new
                        {
                            Id = new Guid("a67fda01-7cee-42dc-a68f-4ca63de64827"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://m.media-amazon.com/images/I/31Vle5fVdaL.jpg"
                        },
                        new
                        {
                            Id = new Guid("7ce42c9f-8388-4002-9853-a1cfb4a088a0"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://adminapi.applegadgetsbd.com/storage/media/large/iPhone-14-Pro-Max-Silver-8456.jpg"
                        },
                        new
                        {
                            Id = new Guid("e4db1184-3379-4337-901d-904355eff527"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_TxUk8mcOtG7_mZmNA6TOZZbADyp3L3HHCg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("1fa85849-c6ba-4a85-b529-2aaf7650d9cd"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_TxUk8mcOtG7_mZmNA6TOZZbADyp3L3HHCg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("4e248407-335d-49d2-8b7e-efb23fc0072b"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSSP2a13umWJp5HCeGtJtw9EuW8opwZfY7r6Q&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("070d41d6-efc2-4ffc-a842-40435ab694c3"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSSP2a13umWJp5HCeGtJtw9EuW8opwZfY7r6Q&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("5b4ba021-743e-4643-abcd-606c05c47832"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSSP2a13umWJp5HCeGtJtw9EuW8opwZfY7r6Q&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("be838efd-0931-4349-9f74-3e91255b8d57"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdUieJcGTqKDqG2pwd8y0PngaUFR0EvjuqPw&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("eb9b921c-b95b-4e81-99dc-0bc713fdde2d"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdUieJcGTqKDqG2pwd8y0PngaUFR0EvjuqPw&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("8c1a7e45-cd69-463b-846d-aa1ba9f69247"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1eifg73dBnUR8qM29W4H7iXk9NCRskq67KQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("e0c11dc5-bb16-460f-85a4-f6dcb46c4507"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1eifg73dBnUR8qM29W4H7iXk9NCRskq67KQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("8bca724b-799f-4212-aed9-18dde3cf3f96"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://dlcdnwebimgs.asus.com/gain/0ad9bdfe-9c75-41fb-82f5-6a6bf25bab22/w800"
                        },
                        new
                        {
                            Id = new Guid("ab757c21-bc5d-4c71-8b50-d10c75da90b1"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://dlcdnwebimgs.asus.com/gain/0ad9bdfe-9c75-41fb-82f5-6a6bf25bab22/w800"
                        },
                        new
                        {
                            Id = new Guid("070719be-a9c8-4327-81b7-8cd4d36874fa"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQc4HSepwRQjDLQitxc9LHPjdKN-8pYQPGlDA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("0c31ff2d-70e9-44c4-b4b1-9be15d03582d"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQc4HSepwRQjDLQitxc9LHPjdKN-8pYQPGlDA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("9ed50c0d-563a-412a-954c-30378ba537f5"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT7lvpJRN10N75r-1ssE_0tpl9dvmOEiKQzXQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("66a8226d-700e-4cd0-913e-8ae8150eef9b"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT7lvpJRN10N75r-1ssE_0tpl9dvmOEiKQzXQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("4e8284be-111c-4a4f-bb8c-96d4e795000c"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTvyCDsCiIgME0mgv_vJYB-285xwWl3bC-pSw&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("edadaf53-2cfa-426d-8bd3-2a3602e069fa"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTvyCDsCiIgME0mgv_vJYB-285xwWl3bC-pSw&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("0e6c331c-bba6-414c-8aed-fe96ddd1b408"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSwSpfSyrQUCFFTMei2zILF0-xFsIYgNgVVSA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("fce08e79-d026-4410-8135-e385dbfcf49e"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSwSpfSyrQUCFFTMei2zILF0-xFsIYgNgVVSA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("76ad8418-2d75-4e1e-8671-05dbcfc46a1f"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXd2u896WCZISe8VyWsmvOQhwbLREi0O93YQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("33a04b0c-44e3-4f30-8c8d-b80d3b7738df"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXd2u896WCZISe8VyWsmvOQhwbLREi0O93YQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("e91ce625-ff0a-49ec-a4fd-822fab55a30d"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ6AmSV387Vy0EVlO6hYPgsKqPnIA_qxq41oA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("d37d4d06-c702-4f23-b4c7-eee26bb26c0b"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ6AmSV387Vy0EVlO6hYPgsKqPnIA_qxq41oA&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("8ed282b4-4f6f-40e2-b54f-49e2e8720f01"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBOB80TIoBkL_NaeJ_5lG5LuUsD0IYEDXFJg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("374e1468-9abd-4fba-a64a-3676468eaeff"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS2zcpFb-6hMeoBVWrfp4LazbLD3a1tEjJjY2oYq45yJOyb28WIIYBoJ3qZMsmFdImBSZk&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("2c7167ef-6591-407c-a5f2-16c86386b358"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1GMCvSHthjghTol1tXK_t4eW7Vk-EuBvdnQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("f4aafad3-b0a2-465f-84d2-c56ad4467526"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1GMCvSHthjghTol1tXK_t4eW7Vk-EuBvdnQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("a80fd49a-46ed-4fad-bf9f-4e649910ea72"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTg7XWy1cmt1D3uUjF29eo2q4HgqJaGEu9XDg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("7e316362-73f3-42ab-9fba-26ec9eb95d2e"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTg7XWy1cmt1D3uUjF29eo2q4HgqJaGEu9XDg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("6cc568a0-b1ed-438a-b643-47daa4ff5db6"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQCEvcavb1VnIbFqFEh_5sgzYPgjcVAbFLl7g&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("4a321a24-59c4-459f-bdeb-2a3c43048be8"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQCEvcavb1VnIbFqFEh_5sgzYPgjcVAbFLl7g&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("ed73c0d6-1879-4911-a249-c3989985f128"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdd3n1bpYxsBGOpZ6ZgUPIKdLVChEegzOBnQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("23162faf-4dc3-4b79-8654-7a492fd63916"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdd3n1bpYxsBGOpZ6ZgUPIKdLVChEegzOBnQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("4662d363-c30b-4270-9584-687e7818b39a"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR42FePhUrLX424aLie09J4Pfkn03T8h3D7PQ&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("21831f7e-4f57-4cb3-8e4a-d3e039b7a117"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.lenovo.com/medias/lenovo-tablet-p10-2.png?context=bWFzdGVyfHJvb3R8NTI0NzIyfGltYWdlL3BuZ3xoM2IvaDFkLzk4MTA1NjI0ODIyMDYucG5nfDI1OGI3YjlkODNkMzI2OThkOWEwMjM1NjliNjI5ZDQ3ZjMxZTg4ZDQ5ZmE5NGY3NTEyYjMwYTA0NDBlM2VhZDA"
                        },
                        new
                        {
                            Id = new Guid("db49b890-cc4a-4812-9bb1-3fcc1ee7847e"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.lenovo.com/medias/lenovo-tablet-p10-2.png?context=bWFzdGVyfHJvb3R8NTI0NzIyfGltYWdlL3BuZ3xoM2IvaDFkLzk4MTA1NjI0ODIyMDYucG5nfDI1OGI3YjlkODNkMzI2OThkOWEwMjM1NjliNjI5ZDQ3ZjMxZTg4ZDQ5ZmE5NGY3NTEyYjMwYTA0NDBlM2VhZDA"
                        },
                        new
                        {
                            Id = new Guid("e8e09518-547c-45e8-9eb4-9d25a88e9b3c"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoJTwIqCk049REtJbZL5xEYO5W9m6iUiSRSg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("91a91674-b0f0-43a4-b05a-e8ebc5e0bfa5"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoJTwIqCk049REtJbZL5xEYO5W9m6iUiSRSg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("98649531-2122-4ca9-af31-553145c445fc"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.istorebangladesh.com/images/thumbs/0000286_macbook-pro-m1_550.png"
                        },
                        new
                        {
                            Id = new Guid("e46f80e7-dbb7-4990-a6e3-933a77d501d3"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.istorebangladesh.com/images/thumbs/0000286_macbook-pro-m1_550.png"
                        },
                        new
                        {
                            Id = new Guid("b5c575a8-f5b3-4129-8f0d-33b4e236e054"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://i0.wp.com/eccocibd.com/wp-content/uploads/2022/01/1802NL02_1.png?fit=550%2C550&ssl=1"
                        },
                        new
                        {
                            Id = new Guid("6c134c6d-bfc3-498a-b201-ffdaf61568d7"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://i0.wp.com/eccocibd.com/wp-content/uploads/2022/01/1802NL02_1.png?fit=550%2C550&ssl=1"
                        },
                        new
                        {
                            Id = new Guid("44de68f8-9b20-4b8f-8eba-939cbcb1264f"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://mirzacdns3.s3.ap-south-1.amazonaws.com/cache/catalog/RLV0015/2-800x800.jpg"
                        },
                        new
                        {
                            Id = new Guid("4d3bd074-3b0b-49a4-8c9f-bc8eb5a92fae"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://mirzacdns3.s3.ap-south-1.amazonaws.com/cache/catalog/RLV0015/2-800x800.jpg"
                        },
                        new
                        {
                            Id = new Guid("c7ee0c23-bb0b-406a-9dd1-2dcad66f7651"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://i0.wp.com/eccocibd.com/wp-content/uploads/2022/01/1802NL02_1.png?fit=550%2C550&ssl=1"
                        },
                        new
                        {
                            Id = new Guid("05345b80-a9a8-4bae-933d-2d57563c80fd"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://i0.wp.com/eccocibd.com/wp-content/uploads/2022/01/1802NL02_1.png?fit=550%2C550&ssl=1"
                        },
                        new
                        {
                            Id = new Guid("ad2f3ae3-5222-41ef-9a6f-1a5955ebbb65"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://m.media-amazon.com/images/I/31Vle5fVdaL.jpg"
                        },
                        new
                        {
                            Id = new Guid("315971a8-aea0-4029-b265-aa95d9896b6a"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://m.media-amazon.com/images/I/31Vle5fVdaL.jpg"
                        },
                        new
                        {
                            Id = new Guid("325a6b1e-51d4-449b-bfa9-f335ff9babc9"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.startech.com.bd/image/cache/catalog/headphone/havit/h763d/h763d-02-500x500.jpg"
                        },
                        new
                        {
                            Id = new Guid("15f4707b-d90b-4827-af2a-bb48544ab3b4"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://eratablet.com/wp-content/uploads/2022/08/H51ba6537405f4948972e293927673546u.jpg"
                        },
                        new
                        {
                            Id = new Guid("f3f36770-c784-4294-a428-1f47b9caa3c0"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://www.bdstall.com/asset/product-image/giant_213181.jpg"
                        },
                        new
                        {
                            Id = new Guid("598bdec5-95ab-4e97-b527-0176b406b2b1"),
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            PublicId = "test",
                            Url = "https://unixnetworkbd.com/image/cache/wp/gj/unix_network_images/product/gaming-pc-9th-gen-core-i7-9700k/Gaming-PC-9th-Gen-Core-i7-9700k-Unix-Computer-jashore-bangladesh-500x500.webp"
                        });
                });

            modelBuilder.Entity("Model.Product", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("CategoryId");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Description");

                    b.Property<int>("DiscountPrice")
                        .HasColumnType("int")
                        .HasColumnName("DiscountPrice");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Name");

                    b.Property<int>("Price")
                        .HasColumnType("int")
                        .HasColumnName("Price");

                    b.Property<int>("Rating")
                        .HasColumnType("int")
                        .HasColumnName("Rating");

                    b.Property<Guid>("ShopId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("ShopId");

                    b.Property<int>("Stock")
                        .HasColumnType("int")
                        .HasColumnName("Stock");

                    b.Property<int>("TotalSell")
                        .HasColumnType("int")
                        .HasColumnName("TotalSell");

                    b.HasKey("Id");

                    b.ToTable("Products", (string)null);
                });

            modelBuilder.Entity("Model.Review", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("Comment")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Comment");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("ProductId");

                    b.Property<int>("Rating")
                        .HasColumnType("int")
                        .HasColumnName("Rating");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("UserId");

                    b.HasKey("Id");

                    b.ToTable("Reviews", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("93ba7588-6338-471f-b0bb-9c4f94034da4"),
                            Comment = "comment",
                            ProductId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Rating = 5,
                            UserId = new Guid("00000000-0000-0000-0000-000000000000")
                        });
                });

            modelBuilder.Entity("Model.Shop", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Name");

                    b.Property<double>("Ratings")
                        .HasColumnType("float")
                        .HasColumnName("Ratings");

                    b.Property<Guid>("ShopAvatarId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("ShopAvatarId");

                    b.HasKey("Id");

                    b.ToTable("Shops", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("1c1438d1-fd47-426c-9a39-7ffa380042d0"),
                            Name = "Apple inc",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("6812dab8-36c8-46d6-9eee-02f3f9a75786")
                        },
                        new
                        {
                            Id = new Guid("569a8312-c0a8-4807-85fd-42293b35fab9"),
                            Name = "Amazon Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("b773d007-2bd8-4ab1-a704-7ac7dbec959a")
                        },
                        new
                        {
                            Id = new Guid("dbeb95e3-ffe5-4c04-9d2a-7ab34dc9fd2a"),
                            Name = "Amazon Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("d5d20b14-ddf1-4626-9a07-e46426c49862")
                        },
                        new
                        {
                            Id = new Guid("7410b8cc-4739-4af1-b7b5-371f245d1307"),
                            Name = "Louis Vuitton",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("5f2f2c95-0e40-41e8-9022-5b3cd3ca9573")
                        },
                        new
                        {
                            Id = new Guid("8154db0c-ae3f-4a85-8439-0e79f66a51d8"),
                            Name = "Shodagor Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("c4d65fa0-255c-4ca9-bc98-84d826677b6b")
                        },
                        new
                        {
                            Id = new Guid("3a62f09b-9c85-45d4-a807-56b290d765a6"),
                            Name = "Amazon Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("2fdb1bcb-3bc6-435c-b8fe-13093fc87a1a")
                        },
                        new
                        {
                            Id = new Guid("412a1e25-5c86-472a-be20-80b635127fa0"),
                            Name = "Asus Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("eda8054b-142a-4ce5-a266-83af8569ddbc")
                        },
                        new
                        {
                            Id = new Guid("84739c97-6473-4548-9aa0-99aed2e57230"),
                            Name = "Acer Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("44c10cb8-643b-4595-9cb5-09b197213b65")
                        },
                        new
                        {
                            Id = new Guid("ae0f0ad6-b4d7-45ce-9fc3-4b5fb9f6ae06"),
                            Name = "Dove Inc",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("d8dcb7b6-97a1-440a-bf75-a945f2907b98")
                        },
                        new
                        {
                            Id = new Guid("53472c47-a418-48d1-969f-ee3ee2862709"),
                            Name = "Dove Inc",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("6bc5b792-2633-463b-8868-835d783c6ec9")
                        },
                        new
                        {
                            Id = new Guid("9fcdaa7c-a530-4d54-ae5f-f45300f4c4b7"),
                            Name = "3W CLINIC",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("dbb9bd99-dee3-403c-8338-1f01f8db3b29")
                        },
                        new
                        {
                            Id = new Guid("312f8ade-2ad6-49b5-835b-300f76d23831"),
                            Name = "Vaseline Inc.",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("74bf8a11-0bfc-4baa-9daa-c6ee9760612b")
                        },
                        new
                        {
                            Id = new Guid("cde6e6d7-7fad-47f3-87fd-ecd7b8ec649e"),
                            Name = "Tiaa Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("a0dfac3f-bc72-4151-9ae6-bc5c60296ea2")
                        },
                        new
                        {
                            Id = new Guid("0133ab03-d17e-42bf-85a2-9181d9d1da61"),
                            Name = "SAMSUNG",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("edfa2996-c98e-4610-8616-4ea276731cd0")
                        },
                        new
                        {
                            Id = new Guid("1a0661ac-c406-493a-a747-93b11b5269c4"),
                            Name = "SAMSUNG",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("fca2bbfc-aef7-44ff-9127-2e122d1ae554")
                        },
                        new
                        {
                            Id = new Guid("248a4afc-2ebc-4547-98f0-e5b9def6f6a2"),
                            Name = "Energy System",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("2e9314db-463f-4871-9013-f3d288ce565d")
                        },
                        new
                        {
                            Id = new Guid("bab213be-033b-4a59-b843-8eae023399a2"),
                            Name = "Unbranded",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("59be1a14-520d-42a7-8e2c-bec3c5c2f196")
                        },
                        new
                        {
                            Id = new Guid("3e914d8c-f8fd-47f7-8233-7a03c4a1295d"),
                            Name = "Unik",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("d60d8d3a-b300-48b4-86b7-0f1808f08941")
                        },
                        new
                        {
                            Id = new Guid("87600fba-1192-46a3-a02c-6fdbf95bb429"),
                            Name = "Oppo Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("7f7a5c64-8abf-4128-b572-9cf67d5c8528")
                        },
                        new
                        {
                            Id = new Guid("fd6b8954-d2d1-482d-9c97-5844a4d08b6c"),
                            Name = "Lenovo Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("d05feba0-59fa-4ef3-9532-abf161421864")
                        },
                        new
                        {
                            Id = new Guid("dc5f9ef2-0351-4dc9-98a2-1b96603bc4e3"),
                            Name = "Unik",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("50c3003b-b3aa-453c-83af-084689da5cc5")
                        },
                        new
                        {
                            Id = new Guid("18f6578c-8f58-43d5-80b7-0a5c01d7db6d"),
                            Name = "Apple inc.",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("e283fadb-5fb9-43a9-b2fc-a1ec8951918a")
                        },
                        new
                        {
                            Id = new Guid("a36a49e3-48e8-45e6-bf16-6e3306136d25"),
                            Name = "Shahriar Watch House",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("ff2c59d4-a590-48bc-b19c-a0ef50b958a4")
                        },
                        new
                        {
                            Id = new Guid("4903b8dc-796f-49f6-b0e5-0afffcf1b6a5"),
                            Name = "Alisha Shoes Mart",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("df48e470-947b-4e31-9392-4fe7302e2fb4")
                        },
                        new
                        {
                            Id = new Guid("bb48ee16-faac-475d-9fde-c2b610a7bb7f"),
                            Name = "Shahriar Watch House",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("1824d3bc-35bb-4846-bae8-62c7651b68b9")
                        },
                        new
                        {
                            Id = new Guid("0360825f-368b-497d-b590-4b82943dc9e0"),
                            Name = "Amazon Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("9a098291-85c7-4901-9cec-9ce919b82335")
                        },
                        new
                        {
                            Id = new Guid("e143c522-fc6d-43e6-9ec9-e338c8353652"),
                            Name = "Asus Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("34ae4315-f07c-48cf-bd85-e87371a2da38")
                        },
                        new
                        {
                            Id = new Guid("5626480e-e493-4665-a49b-33f0df80ea23"),
                            Name = "Amazon Ltd",
                            Ratings = 4.0,
                            ShopAvatarId = new Guid("76939832-8847-4b22-9564-b43b68ce5146")
                        });
                });

            modelBuilder.Entity("Model.ShopAvatar", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("Public_id")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("PublicId");

                    b.Property<Guid>("ShopId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("ShopId");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Url");

                    b.HasKey("Id");

                    b.ToTable("ShopAvatars", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("5800ffbc-ec6e-4031-98b6-d659f5d5e384"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("c53b9419-0cf5-4f8d-bc15-91eb888c86cd"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("9707ffca-652f-4766-8cbc-c68574184535"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("4f551209-a672-4d5c-86db-80a521e930ab"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQeGWw-VaNcL9QlsHKJKPLWj-Dh0M2WguDpRg&usqp=CAU"
                        },
                        new
                        {
                            Id = new Guid("3fdef302-24da-467d-8705-2c7ce1c42a61"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("63c4b689-17f8-4f76-b237-ea8e2c0b8f62"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("8e9d0eae-19e7-4067-a32c-6e663a8f2441"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("e8180110-eb59-46df-ab3e-2a55005edca5"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("5796dbe8-95b7-4a21-88e5-8892127d6744"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("7915518a-becf-4c04-9b12-fee1d807727a"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("f516859b-cc34-40f7-84d3-9fc03d46cddd"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("c7ba492d-8e32-4f92-ae64-3381041bc765"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("d59576db-58ad-453e-a04f-a1706e945e5c"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("ae6571e3-fbb1-475a-b223-fe1decde716f"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/jpeg;base64,"
                        },
                        new
                        {
                            Id = new Guid("a4ee5a76-5a85-48c1-8cfd-7001acaa7714"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("b4c6e5b5-88d3-4d66-b22d-5e3cc068914d"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("58e75fac-d100-4b8f-9aa5-e02257702029"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("565f7fe5-c1de-455d-be25-56a6648b7948"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("6caaa6fd-792a-48d0-b8eb-390668dd8aac"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("50e2f411-5188-40c0-9c9c-4985779f604f"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("02403ca7-c44f-4ffa-b75e-22347d29f1b4"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "data:image/png;base64,"
                        },
                        new
                        {
                            Id = new Guid("a46c6035-4ed7-43cd-a1ad-ecf4d7c2f273"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("cc6cc44d-f395-4c0e-8819-ba62c2d2ee42"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("6146f98e-346d-4bda-b1b5-a54acea98e0a"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("f4d7ee74-477d-45fe-80b5-b569c81f99eb"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("d08549d3-28c2-4a85-91bb-3be22f04f4a8"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("f71c1802-a472-4b52-ba8e-ca896f418e23"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        },
                        new
                        {
                            Id = new Guid("d7bea74e-1675-45a0-a51c-344305d3d23f"),
                            Publicid = "test",
                            ShopId = new Guid("00000000-0000-0000-0000-000000000000"),
                            Url = "https://www.hatchwise.com/wp-content/uploads/2022/05/amazon-logo-1024x683.png"
                        });
                });

            modelBuilder.Entity("Model.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("Id");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<int>("Age")
                        .HasColumnType("int")
                        .HasColumnName("Age");

                    b.Property<DateTime>("Birthday")
                        .HasColumnType("datetime2")
                        .HasColumnName("Birthday");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)")
                        .HasColumnName("Email");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("FullName");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Gender");

                    b.Property<string>("Hobby")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Hobby");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("RefreshToken")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("RefreshTokenExpiryTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("Users", (string)null);

                    b.HasData(
                        new
                        {
                            Id = "515775a3-c22c-48eb-8b7b-cd4cf711e2b6",
                            AccessFailedCount = 0,
                            Age = 36,
                            Birthday = new DateTime(2024, 6, 14, 16, 43, 37, 989, DateTimeKind.Local).AddTicks(3001),
                            ConcurrencyStamp = "68bec36d-2bd8-43c0-a23f-b08f6af48415",
                            Email = "oleitao@gmail.com",
                            EmailConfirmed = false,
                            FullName = "oliveira leitao",
                            Gender = "M",
                            Hobby = "netflix",
                            LockoutEnabled = false,
                            PhoneNumberConfirmed = false,
                            RefreshTokenExpiryTime = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            SecurityStamp = "dd33c10e-f5d0-40f8-8ed7-9376d7231a35",
                            TwoFactorEnabled = false
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
